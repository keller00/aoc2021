...
PairOrInt = TypeVar("PairOrInt", Pair, int)
...
def solve(_input: str) -> int:
    ...
    for a, b in product(input_lines, input_lines):
        if a != b:
            placeholder = dict()  # type: ignore
            exec(f"a={a}", {}, placeholder)
            exec(f"b={b}", {}, placeholder)
            a_list = turn_lists_into_pairs(placeholder["a"])
            b_list = turn_lists_into_pairs(placeholder["b"])
            assert isinstance(a_list, Pair)
            assert isinstance(b_list, Pair)
            add_parents(a_list)
            add_parents(b_list)
            magnitude = max(calculate_magnitude(
                add(a_list, b_list)), magnitude)
...
@pytest.mark.parametrize(("input_file", "expected_result"), [
    (this_dir / "sample_input.txt", 3993),
    (this_dir / "input.txt", 4837),
])
