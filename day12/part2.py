...
def solve(_input: str) -> int:
    ...
    def calculate_paths(
        ...
        # Note: current_path could be a set, this would make the program run
        #  much faster, but that way we could not actually calculate the
        #  paths, only count them which is the point of this challenge, but
        #  for debugging reasons I wanted to actually calculate the paths
        ...
        for neighbour_cave in cave.neighbours:
            if neighbour_cave.big or neighbour_cave not in current_path:
                calculate_paths(neighbour_cave, current_path_copy,
                                all_paths, visited_small_cave_twice)
            elif (not visited_small_cave_twice and neighbour_cave not in (
                    caves["start"],
                    caves["end"],
            )):
                calculate_paths(
                    neighbour_cave, current_path_copy, all_paths, True)

    _all_paths: list[list[Cave]] = []
    calculate_paths(caves["start"], [], _all_paths, False)
...
@pytest.mark.parametrize(("input_file", "expected_result"), [
    (this_dir / "sample_input.txt", 36),
    (this_dir / "input.txt", 117095),
])
